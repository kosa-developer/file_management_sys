...
if (isset($_GET['code'])) {
    ...
    $staffCheck = "SELECT staff.Image,CONCAT(staff.Title,'. ',staff.Staff_Name) AS Full_Name FROM policy_codes,staff WHERE policy_codes.Staff_Id=staff.Staff_Id AND policy_codes.Code='$staff_code' LIMIT 1";
    $staff_list = DB::getInstance()->query($staffCheck);
    $staff_id = DB::getInstance()->getName("policy_codes", $staff_code, "Staff_Id", "Code");
